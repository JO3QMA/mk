...
// startedGameForReaction creates a pre-started reversi_game row that the
// reaction tests can target without going through UpdateReady (which itself
// publishes federation Updates we'd have to count around).
func startedGameForReaction(t *testing.T) (*model.ReversiGame, *fakeRepo, *capturePublisher, *Service) {
	t.Helper()
	repo := newFakeRepo()
	pub := &capturePublisher{}
	svc := NewService(repo, pub, nil)
...
func TestService_SendReaction_LocalActor_PublishesAndDelivers(t *testing.T) {
	game, _, pub, svc := startedGameForReaction(t)
	d := wireFederated(t, svc, game, "sess-react")

	require.NoError(t, svc.SendReaction(context.Background(), game.ID, "alice", ":heart:"))

	// `reacted` イベントが game stream に publish される
	kinds := pub.types()
	assert.Contains(t, kinds, "reacted")

	// remote bob 宛に EmojiReaction が配送される
	require.Len(t, d.calls, 1)
	var act map[string]any
	require.NoError(t, json.Unmarshal(d.calls[0].body, &act))
	assert.Equal(t, "EmojiReaction", act["type"])
	assert.Equal(t, ":heart:", act["content"])
	assert.Equal(t, ":heart:", act["_misskey_reaction"])
	// object は CherryPick 互換の `/games/{UUID}/{sessionID}` URI
	obj, ok := act["object"].(string)
	require.True(t, ok)
	assert.Contains(t, obj, "/games/"+GameTypeUUID+"/sess-react")
}

func TestService_SendReaction_EmptyEmojiFallsBackToHeart(t *testing.T) {
	game, _, pub, svc := startedGameForReaction(t)
	wireFederated(t, svc, game, "sess-empty")

	require.NoError(t, svc.SendReaction(context.Background(), game.ID, "alice", ""))

	// publish された body に default emoji ❤️ が入っていることを確認
	body := pub.latestBody()
	require.NotNil(t, body)
	m, ok := body.(map[string]any)
	require.True(t, ok)
	assert.Equal(t, "❤️", m["reaction"])
}

func TestService_SendReaction_RemoteActor_NoEchoBackDeliver(t *testing.T) {
	// 受信した EmojiReaction で SendReaction が再呼出された場合、actor が
	// remote (bob) なので outbound deliver は走らない (echo 防止)。WS publish
	// は両プレイヤー宛なので走ってよい。
	game, _, pub, svc := startedGameForReaction(t)
	d := wireFederated(t, svc, game, "sess-echo")

	require.NoError(t, svc.SendReaction(context.Background(), game.ID, "bob", ":fire:"))

	assert.Contains(t, pub.types(), "reacted")
	assert.Empty(t, d.calls, "remote actor reaction must not be echoed back")
}

func TestService_SendReaction_PreStartGameNoOp(t *testing.T) {
	// pre-start: SendReaction は何もしない (CherryPick 仕様)。
	game, _, pub, svc := newPendingGame(t)
	d := wireFederated(t, svc, game, "sess-prestart")

	require.NoError(t, svc.SendReaction(context.Background(), game.ID, "alice", ":heart:"))

	assert.NotContains(t, pub.types(), "reacted")
	assert.Empty(t, d.calls)
}

func TestService_SendReaction_NonPlayerNoOp(t *testing.T) {
	game, _, pub, svc := startedGameForReaction(t)
	d := wireFederated(t, svc, game, "sess-stranger")

	require.NoError(t, svc.SendReaction(context.Background(), game.ID, "carol", ":heart:"))
