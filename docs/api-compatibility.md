...
- **Playwright e2e**: 294 spec ファイルを PR ごとに実行。Misskey TS backend に対しては upstream 追従時に実行し、spec が mk-go の挙動に引きずられていないかを検証する
...
プロトコル改善 (#132):
- OAuth2スコープに基づくチャンネルアクセス制御 (チャンネル単位の判定が実際に
  効くようになったのは 1.2.1 より後。それ以前は接続にスコープが載っておらず
  素通ししていた)
- `pong`応答の実装
- パラメータバリデーション強化
...
Go側のマイグレーション (000001〜) はTS版テーブルに対して原則追加のみだが、例外が 11 件ある ([migration-from-ts.md](migration-from-ts.md#破壊的なマイグレーション))。TS版のマイグレーションで作成される全テーブルは維持される。
